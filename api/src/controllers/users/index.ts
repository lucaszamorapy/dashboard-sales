...
import { alterUser, createUser, login, validToken } from '../../services/users';
...
export const validTokenController = (req: AuthenticatedRequest, res: Response): void => {
  try {
    const response = validToken(req.body)
    res.status(200).json(response);
  } catch (error: any) {
    const message = error.message.replace(/^Error:\s*/, "");
    res.status(400).json({ error: message });
